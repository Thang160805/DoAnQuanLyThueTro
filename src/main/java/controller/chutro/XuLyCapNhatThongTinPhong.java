...
/**
 * Servlet implementation class XuLyCapNhatThongTinPhong
 */
@WebServlet("/XuLyCapNhatThongTinPhong")
@MultipartConfig(
        fileSizeThreshold = 1024 * 1024 * 1,    // 1 MB
        maxFileSize = 1024 * 1024 * 20,         // 20 MB
        maxRequestSize = 1024 * 1024 * 50       // 50 MB
)
public class XuLyCapNhatThongTinPhong extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public XuLyCapNhatThongTinPhong() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        pt.setID_Phong(ID_Phong);
        boolean checked = ptBO.UpdatePhongTro(pt);
        if(checked) {
        	ptBO.deletePhongTro_TienIch(ID_Phong);
        	ptBO.deleteHinhAnhPhong(ID_Phong);
